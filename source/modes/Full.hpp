    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        auto Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
            ...
            if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck)) {
                ...
                renderer->drawString("CPU 使用率", false, COMMON_MARGIN, 120, 20, (settings.catColor1));
                if (settings.showTargetFreqs) {
                    //static auto targetFreqWidth = renderer->getTextDimensions("目标频率: ", false, 15).first;
                ...
                if (realCPU_Hz && settings.showRealFreqs) {
                    //static auto realFreqWidth = renderer->getTextDimensions("实际频率: ", false, 15).first;
                ...
                renderer->drawString("核心 1  ", false, COMMON_MARGIN, core_height + 15, 15, (settings.catColor2));
                renderer->drawString(CPU_Core1_c, false, COMMON_MARGIN + core1Width, core_height + 15, 15, (settings.textColor));
                
                renderer->drawString("核心 2  ", false, COMMON_MARGIN, core_height + 30, 15, (settings.catColor2));
                renderer->drawString(CPU_Core2_c, false, COMMON_MARGIN + core2Width, core_height + 30, 15, (settings.textColor));
                
                renderer->drawString("核心 3  ", false, COMMON_MARGIN, core_height + 45, 15, (settings.catColor2));
            ...
            if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck) || R_SUCCEEDED(nvCheck)) {
                ...
                renderer->drawString("GPU 使用率", false, COMMON_MARGIN, 285-8, 20, (settings.catColor1));
                if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck)) {
                    if (settings.showTargetFreqs) { 
                        //static auto targetFreqWidth = renderer->getTextDimensions("目标频率: ", false, 15).first;
                        renderer->drawString("目标频率", false, COMMON_MARGIN, height_offset, 15, (settings.catColor2));
                ...
                if (R_SUCCEEDED(nvCheck)) {
                    //static auto loadWidth = renderer->getTextDimensions("负载: ", false, 15).first;
            ...
            if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck) || R_SUCCEEDED(Hinted)) {
                ...
                renderer->drawString("内存 使用率", false, COMMON_MARGIN, 375, 20, (settings.catColor1));
                if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck)) {
                    if (settings.showTargetFreqs) {
                        //static auto targetFreqWidth = renderer->getTextDimensions("目标频率: ", false, 15).first;
                        renderer->drawString("目标频率", false, COMMON_MARGIN, height_offset, 15, (settings.catColor2));
                ...
                if (R_SUCCEEDED(Hinted)) {
                    //static auto textWidth = renderer->getTextDimensions("总计: \n应用: \n小程序: \n系统: \n系统保留: ", false, 15).first;
                    renderer->drawString("总计: \n应用: \n小程序: \n系统: \n系统保留: ", false, COMMON_MARGIN, height_offset + 40, 15, (settings.catColor2));
            ...
            if (R_SUCCEEDED(i2cCheck) || R_SUCCEEDED(tcCheck) || R_SUCCEEDED(pwmCheck)) {
                renderer->drawString("主板", false, 20, 550+2, 20, (settings.catColor1));
                ...
                if (R_SUCCEEDED(i2cCheck) || R_SUCCEEDED(tcCheck)) {
                    static auto socLabelWidth = renderer->getTextDimensions("核心 ", false, 15).first;
                    static auto pcbLabelWidth = renderer->getTextDimensions("主板 ", false, 15).first;
                    static auto maxLabelWidth = std::max(socLabelWidth, pcbLabelWidth);
                    static auto skinLabelWidth = renderer->getTextDimensions("壳温 ", false, 15).first;
                    ...
                    renderer->drawString("核心 ", false, current_x, 605+2, 15, (settings.catColor2));
                    ...
                    renderer->drawString("壳温 ", false, current_x, 605+2, 15, (settings.catColor2));
                    ...
                    renderer->drawString("主板 ", false, current_x, 620+2, 15, (settings.catColor2));
                    current_x += maxLabelWidth;
            ...
            if (GameRunning) {
                ...
                if (settings.showFPS || settings.showRES || settings.showRDSD) {
                    renderer->drawString("游戏", false, COMMON_MARGIN + width_offset, 185+12, 20, (settings.catColor1));
                }
                ...
                if (settings.showFPS == true) {
                    ...
                    renderer->drawString("帧率  ", false, fps_x_offset, height, 15, (settings.catColor2));
                ...
                if ((settings.showRES == true) && NxFps && SharedMemoryUsed && (NxFps -> API >= 1)) {
                    static auto resLabelWidth = renderer->getTextDimensions("分辨率  ", false, 15).first;
                    renderer->drawString("分辨率  ", false, COMMON_MARGIN + width_offset, height, 15, (settings.catColor2));
                ...
                if (settings.showRDSD == true) {
                    static auto readLabelWidth = renderer->getTextDimensions("读取速度  ", false, 15).first;
                    renderer->drawString("读取速度  ", false, COMMON_MARGIN + width_offset, height, 15, (settings.catColor2));
            ...
            static const auto pressWidth = renderer->getTextDimensions("按住", false, 23).first;
            ...
            // Draw "Press "
            renderer->drawString("按住", false, baseX, baseY, fontSize, (tsl::bottomTextColor));
        ...
        auto rootFrame = new tsl::elm::HeaderOverlayFrame("性能监控", APP_VERSION);
