    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        auto Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
            ...
            if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck) || R_SUCCEEDED(Hinted)) {
                ...
                if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck)) {
                    ...
                    if (R_SUCCEEDED(sysclkCheck)) {
                        //static auto loadLabelWidth = renderer->getTextDimensions("负载: ", false, 15).first;
                        renderer->drawString("负载", false, COMMON_MARGIN, height_offset+15, 15, (settings.catColor2));
            ...
            if (R_SUCCEEDED(i2cCheck) || R_SUCCEEDED(tcCheck) || R_SUCCEEDED(pwmCheck)) {
                renderer->drawString("主板", false, 20, 550, 20, (settings.catColor1));
                if (R_SUCCEEDED(i2cCheck)) {
                    //static auto batteryLabelWidth = renderer->getTextDimensions("Battery Power Flow: ", false, 15).first;
                    renderer->drawString("电池功率", false, COMMON_MARGIN, 575, 15, (settings.catColor2));
                ...
                if (R_SUCCEEDED(i2cCheck) || R_SUCCEEDED(tcCheck)) {
                    ...
                    static auto textWidth2 = renderer->getTextDimensions("核心\n主板\n壳温", false, 15).first;
                    renderer->drawString("\n温度", false, COMMON_MARGIN, 590, 15, (settings.catColor2));
                    renderer->drawString("核心\n主板\n壳温", false, COMMON_MARGIN + valueOffset-textWidth2 - 10, 590, 15, (settings.catColor2));
                    renderer->drawString(SoCPCB_temperature_c, false, COMMON_MARGIN + valueOffset, 590, 15, (settings.textColor));
                }
                if (R_SUCCEEDED(pwmCheck)) {
                    //static auto fanLabelWidth = renderer->getTextDimensions("Fan Rotation Level ", false, 15).first;
                    renderer->drawString("风扇转速", false, COMMON_MARGIN, 635, 15, (settings.catColor2));
