...
public:
            ...
            if (R_SUCCEEDED(nifmCheck)) {
                ...
                if (!Nifm_internet_rc) {
                    if (NifmConnectionType == NifmInternetConnectionType_WiFi) {
                        renderer->drawString("网络连接: WIFI", false, 20, 280, 18, renderer->a(0xFFFF));
                        if (!Nifm_profile_rc) {
                            if (Nifm_showpass)
                        ...
                        renderer->drawString("网络连接: 有线", false, 20, 280, 18, renderer->a(0xFFFF));
                }
                else
                    renderer->drawString("网络连接: 未连接", false, 20, 280, 18, renderer->a(0xFFFF));
            }
    ...
    virtual void update() override {
        ...
        snprintf(DSP_Load_c, sizeof DSP_Load_c, "音频处理占用: %u%%", DSP_Load_u);
        snprintf(NVDEC_Hz_c, sizeof NVDEC_Hz_c, "硬件视频解码: %.1f MHz", (float)NVDEC_Hz / 1000000);
        snprintf(NVENC_Hz_c, sizeof NVENC_Hz_c, "硬件视频编码: %.1f MHz", (float)NVENC_Hz / 1000000);
        snprintf(NVJPG_Hz_c, sizeof NVJPG_Hz_c, "图片编解码器: %.1f MHz", (float)NVJPG_Hz / 1000000);
        char pass_temp1[25] = "";
        char pass_temp2[25] = "";
        char pass_temp3[17] = "";
