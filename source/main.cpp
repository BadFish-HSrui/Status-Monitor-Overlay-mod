...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        list->addItem(new tsl::elm::CategoryHeader("其他"));
...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        if (fileExist) {
            ...
            auto* Micro = new tsl::elm::ListItem("微型");
        ...
        if (SaltySD) {
            ...
            auto* comFPSGraph = new tsl::elm::ListItem("FPS图表");
            comFPSGraph->setClickListener([](uint64_t keys) {
                if (keys & KEY_A) {
                    tsl::changeTo<com_FPSGraph>();
                    return true;
            ...
            auto* comFPSCounter = new tsl::elm::ListItem("FPS计数");
            comFPSCounter->setClickListener([](uint64_t keys) {
                if (keys & KEY_A) {
                    tsl::changeTo<com_FPS>();
                    return true;
